...
trait IRAnnotationReader {
  ...
  trait IRAnnotationReaders {
    ...
    private lazy val methodSignatureAnnotation: IRAnnotation =?> JMethodSignature = for {
      ...
      exceptions  <- array("throwsTypes")(classTypeSignature)
      activates   <- array("activates")(classTypeSignature)
      deactivates <- array("deactivates")(classTypeSignature)
      requires    <- array("requires")(classTypeSignature)
