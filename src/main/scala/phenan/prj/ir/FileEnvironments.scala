...
trait FileEnvironments {
  this: Environments with NameResolvers with JClassLoader with IRs with JModules with JErasedTypes with Application =>
  ...
  case class FileEnvironment(file: IRFile) {
    ...
    private def tsort(priorities: Map[JPriority, Set[JPriority]], sorted: List[JPriority]): List[JPriority] = priorities.find(_._2.isEmpty) match {
      ...
      case None =>
        if (priorities.nonEmpty) error("invalid priority : cyclic precedence")
        sorted.reverse
    }
  }
}