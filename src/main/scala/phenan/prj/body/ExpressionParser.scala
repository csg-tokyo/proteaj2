...
trait ExpressionParser {
  this: ExpressionOperatorParser with JavaExpressionParser with LiteralParser with JTypeLoader with Environments with IRExpressions with JModules =>
  ...
  trait ExpressionParsers {
    ...
    private class ExpressionParsersImpl(expected: JType, env: Environment) extends ExpressionParsersInterface {
      ...
      lazy val javaExpression: HParser[IRExpression] = getJavaExpressionParser(env) ^? { case e if e.staticType.exists(_ <:< expected) || expected == voidType => e }
