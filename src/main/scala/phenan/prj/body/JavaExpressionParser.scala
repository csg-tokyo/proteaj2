...
trait JavaExpressionParser {
  this: ArgumentParser with ExpressionParser with TypeParser with JTypeLoader with Environments with IRs with IRExpressions with JModules with JMembers with JErasedTypes =>
  ...
  trait JavaExpressionParsers {
    ...
    private class JavaExpressionParsersImpl(env: Environment) extends JavaExpressionParsersInterface {
      ...
      private def anonymousClass_Constructors(metaArgs: List[MetaArgument], baseType: JObjectType, constructors: List[JConstructor]): Option[HParser[IRAnonymousClass]] = constructors match {
        case Nil if metaArgs.isEmpty => Some(anonymousClassBody ^^ {
          IRAnonymousClass(Map.empty, baseType, Nil, Nil, _)
        })
        case cs => cs.flatMap {
          anonymousClass_Constructor(metaArgs, baseType, _)
        }.reduceOption(_ | _)
      }
      ...
      lazy val thisConstructorCall: HParser[IRThisConstructorCall] = typeParsers.metaArguments <~ "this" >>? { metaArgs =>
        env.thisType.flatMap {
          _.findConstructor(env.clazz).flatMap(thisConstructorCall(metaArgs, _)).reduceOption(_ | _)
        }
      }

      lazy val superConstructorCall: HParser[IRSuperConstructorCall] = typeParsers.metaArguments <~ "super" >>? { metaArgs =>
        env.thisType.flatMap(_.superType).flatMap {
          _.findConstructor(env.clazz).flatMap(superConstructorCall(metaArgs, _)).reduceOption(_ | _)
        }
      }
      ...
      lazy val instanceMethodCall: HParser[IRInstanceMethodCall] = primary ~ ('.' ~> typeParsers.metaArguments) ~ identifier >>? {
        case instance ~ metaArgs ~ name => instance.staticType.flatMap {
          _.findMethod(name, env.clazz, isThisRef(instance)).flatMap(invokeVirtual(instance, metaArgs, _)).reduceOption(_ | _)
        }
      }
      ...
      lazy val instanceFieldAccess: HParser[IRInstanceFieldAccess] = primary ~ ('.' ~> identifier) ^^? {
        case instance ~ name => instance.staticType.flatMap {
          _.findField(name, env.clazz, isThisRef(instance)).map(IRInstanceFieldAccess(instance, _))
        }
      }
      ...
      lazy val variableRef: HParser[IRLocalVariableRef] = identifier ^^? {
        env.localVariable
      }

      lazy val intExpression: HParser[IRExpression] = getExpressionParser(intType, env)

      lazy val dimension: HParser[Int] = ('[' ~> ']').* ^^ {
        _.length
      }

      lazy val dimension1: HParser[Int] = ('[' ~> ']').+ ^^ {
        _.length
      }
      ...
      private def procedureCall[T](procedure: JProcedure, metaArgs: List[MetaArgument])(f: (Map[String, MetaArgument], List[IRExpression], List[IRContextRef]) => T): Option[HParser[T]] = {
        ...
        } yield getArgumentsParser(procedure, bind, env) ^^ {
          f(bind, _, contexts)
        }
      }
      ...
      private def superConstructorCall(metaArgs: List[MetaArgument], constructor: JConstructor): Option[HParser[IRSuperConstructorCall]] =
        procedureCall(constructor, metaArgs) {
          IRSuperConstructorCall(_, constructor, _, _)
        }

      private def invokeVirtual(instance: IRExpression, method: JMethod): HParser[IRInstanceMethodCall] =
        procedureCall(method) {
          IRInstanceMethodCall(instance, _, method, _, _)
        }
      ...
      private def invokeSpecial(thisType: JObjectType, metaArgs: List[MetaArgument], method: JMethod): Option[HParser[IRSuperMethodCall]] =
        procedureCall(method, metaArgs) {
          IRSuperMethodCall(thisType, _, method, _, _)
        }

      private def invokeStatic(method: JMethod): HParser[IRStaticMethodCall] =
        procedureCall(method) {
          IRStaticMethodCall(_, method, _, _)
        }
