...
trait StatementParser {
  this: ExpressionParser with JavaExpressionParser with TypeParser with JTypeLoader with Environments with IRStatements with IRExpressions with JModules with Application =>
  ...
  trait StatementParsers {
    ...
    private class StatementParsersImpl(returnType: JType, env: Environment) extends StatementParsersInterface {
      ...
      lazy val blockStatements: HParser[List[IRStatement]] = blockStatement >> {
        case s@IRLocalDeclarationStatement(local) => getStatementParsers(returnType, env.defineLocals(local)).blockStatements ^^ {
          s :: _
        }
        case s: IRExpressionStatement => getStatementParsers(returnType, env.modifyContext(s)).blockStatements ^^ {
          s :: _
        }
        case s => blockStatements ^^ {
          s :: _
        }
      } | HParser.success(Nil)
      ...
      lazy val ifStatement: HParser[IRIfStatement] = ("if" ~> '(' ~> expression(booleanType) <~ ')') ~ statement ~ ("else" ~> statement).? ^^ {
        case cond ~ thenStmt ~ elseStmt => IRIfStatement(cond, thenStmt, elseStmt)
      }

      lazy val whileStatement: HParser[IRWhileStatement] = ("while" ~> '(' ~> expression(booleanType) <~ ')') ~ statement ^^ {
      ...
      protected[StatementParsers] lazy val forControlRest: HParser[Option[IRExpression] ~ List[IRExpression] ~ IRStatement] = expression(booleanType).? ~ (';' ~> statementExpressionList) ~ (')' ~> statement)
      ...
      lazy val exceptionHandler: HParser[IRExceptionHandler] = ("catch" ~> '(' ~> typeName) ~ (identifier <~ ')') >> {
        case t ~ id => getStatementParsers(returnType, env.defineLocal(t, id)).block ^^ {
          IRExceptionHandler(t, id, _)
        }
      }
