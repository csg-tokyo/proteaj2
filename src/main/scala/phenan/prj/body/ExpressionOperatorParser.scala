...
trait ExpressionOperatorParser {
  this: ExpressionOperandParser with Unifier with Environments with DSLEnvironments with IRs with IRExpressions with Syntax with JModules with JMembers with JErasedTypes =>
  trait ExpressionOperatorParsers {
    ...
    private class ExpressionOperatorParsersImpl(eop: ExpressionOperator, env: Environment) extends ExpressionOperatorParsersInterface {
      ...
      private def constructParser(pattern: List[JSyntaxElement], binding: Map[String, MetaArgument], operands: List[IRExpression]): HParser[IRExpression] = pattern match {
        ...
        case JOptionalOperand(param, p) :: rest => getExpressionOperandParser(param, p, binding, eop.method, env).?.mapOption {
          _.orElse(defaultArgument(param, eop.method, env))
        } >> { arg =>
          constructParser(rest, bind(param, arg, binding), arg :: operands)
        }
        ...
        case JRegexName(name) :: rest => regex(name).^ >> { s => constructParser(rest, binding, IRStringLiteral(s) :: operands) }
