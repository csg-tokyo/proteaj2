...
trait DescriptorParser {
  ...
  object DescriptorParsers extends PackratParsers {
    ...
    lazy val methodDesc: PackratParser[JMethodSignature] =
      ('(' ~> typeDesc.* <~ ')') ~ returnDesc ^^ { case paramTypes ~ retType => JMethodSignature(Nil, paramTypes.map(sig => JParameterSignature(Nil, Nil, sig, false, None, Nil)), retType, Nil, Nil, Nil, Nil, Nil) }
